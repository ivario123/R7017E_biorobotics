...
fn task4() {
    let p0_1 = Pose::<0, 1>::from_translation(Vector::new_from_data([3f32, 0f32, 0f32]));
    let p1_2 = Pose::<1, 2>::rot(rotz(PI / 2f32).unwrap());
    let (rot, _t) = p1_2.clone().decompose();

    let translation = rot.clone().transpose() * Vector::new_from_data([0f32, 1f32, 0f32]);
    let p2_3 = Pose::<2, 3>::from_translation(translation);
    println!("p2_3:\n{p2_3}");
...
fn task3() {
    ...
    println!(
        ...
        r1.to_tex(Some("R_1")),
        r2.to_tex(Some("R_2")),
        (r1 * r2).to_tex(Some("R_{composite}"))
    );
    println!(".m");
